...
import { Card, CardHeader, CardBody, Box, Heading, Text, SimpleGrid, Button } from '@chakra-ui/react';
...
const GalleryUi = ({ artworks, addArtwork, deleteArtwork, setArtworks }) => {
    ...
    return (
        <div>
            <Button onClick={triggerFileUpload} colorScheme="teal" mb={4}>Upload Artwork</Button>
            ...
            <SimpleGrid spacing={2} templateColumns={{ base: "repeat(3, 1fr)", md: "repeat(5, 1fr)", lg: "repeat(7, 1fr)" }}>
                {artworks.map(artwork => (
                    <Card 
                        ...
                        width="100%"  // Use 100% so it uses the full width of the grid cell
                        _hover={{ ".card-content": { opacity: 1, transform: "scale(1)" } }}
                    >
                        <Box position="absolute" top="2" right="2" zIndex="1">
                            <Button size="sm" colorScheme="red" onClick={() => handleDelete(artwork.id, artwork.title)}>X</Button>
                        </Box>
...
const mapDispatchToProps = {
    addArtwork,
    deleteArtwork,
    setArtworks // This action is for populating the Redux store with artworks fetched from Firebase.
};
